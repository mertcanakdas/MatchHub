...
class upload {
    ...
    function getsize($size) {
        // Girdi verisinin geçerli bir string olduğundan emin ol
        if (!is_string($size) || strlen($size) === 0) {
            return 0; // Uygun bir varsayılan değer döndür
        }
    
        // Son karakteri al ve küçük harfe çevir
        $last = strtolower($size[strlen($size) - 1]);
    
        // Boyutun sayısal kısmını al
        $size = (int) $size;
    
        // Birimi kontrol et ve uygun çarpanı uygula
        switch ($last) {
            case 'g':
                $size *= 1024 * 1024 * 1024; // GB -> Byte
                break;
            case 'm':
                $size *= 1024 * 1024; // MB -> Byte
                break;
            case 'k':
                $size *= 1024; // KB -> Byte
                break;
    ...
    function check_upload_size($file_size, $max_size) {
        if ($file_size > $max_size) {
            return false; // Dosya boyutu izin verilen maksimum boyutu aşıyor
        }
        return true;
    }
    
    // Dosya yükleme fonksiyonunda güncellemeler yapıldı
    function process_upload($file, $target_dir, $allowed_types, $max_size) {
        // Dosya hatasını kontrol et
        if ($file['error'] !== UPLOAD_ERR_OK) {
            return [
                'success' => false,
                'error' => 'Dosya yüklenirken bir hata oluştu: ' . $file['error']
            ];
        }
    
        // Dosya boyutunu kontrol et
        if (!check_upload_size($file['size'], $max_size)) {
            return [
                'success' => false,
                'error' => 'Dosya boyutu izin verilen maksimum boyutu aşıyor (' . $max_size . ' byte)'
            ];
        }
        ...
        if (!in_array($file_type, $allowed_types)) {
            return [
                'success' => false,
                'error' => 'Geçersiz dosya türü: ' . $file_type
            ];
        }
