var url = "http://localhost/MatchHub";
  ...
  $.ajax({
    ...
    success: function (result) {
      if ($.trim(result) == "empty") {
        ...
        alert(
          "Üyeliğiniz başarıyla oluştuldu..."
        );
  ...
  $.ajax({
    ...
    success: function (result) {
      if ($.trim(result) == "empty") {
        ...
        alert(
          "Havale bildiriminiz gönderildi, yönetici kontrolünden sonra tarafınıza ulaşım sağlanacaktır"
        );
  ...
  $.ajax({
    ...
    success: function (result) {
      if ($.trim(result) == "empty") {
        ...
        alert("Yorumunuz en az 5 karakter olmalıdır...");
        document.getElementById("newcommentt").disabled = false;
      } else if ($.trim(result) == "ok") {
  ...
  $.ajax({
    ...
    success: function (result) {
      if ($.trim(result) == "empty") {
        ...
        alert("Bu bilgilere ait kullanıcı bulunmuyor");
        document.getElementById("passwordbuton2").disabled = false;
      } else if ($.trim(result) == "ok") {
